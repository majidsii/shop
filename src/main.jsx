        ...
        return (<div key={item.name} className='flex-1 rounded-lg border-[1px] bg-[#ffffff] overflow-hidden min-w-[400px]'>
          ...
          <div className='flex flex-col h-48 bg-gray-200 rounded-t-2xl p-2'>
            ...
            <div className='flex gap-1 items-center'>
              ...
              {buyItemCount !== 0 && <button disabled={item.mojodi === 0} onClick={() => { decreseItem(item); }} className='w-6 disabled:bg-slate-300 cursor-pointer my-2 rounded-md bg-[#30303a] text-white text-sm p-1'>-</button>}

              {buyItemCount === 0 && <button disabled={item.mojodi === 0} onClick={() => { buyItem(item); }} className='flex-1 disabled:bg-slate-300 cursor-pointer my-2 rounded-md bg-[#30303a] text-white text-sm p-1'>خرید کالا</button>}

              {buyItemCount !== 0 && <button disabled={item.mojodi === 0} onClick={() => {
                if (buyItemCount >= item.mojodi) return alert("موجودی این کالا تمام شد");
                buyItem(item)
              }} className='flex-1 disabled:bg-slate-300 cursor-pointer my-2 rounded-md bg-[#30303a] text-white text-sm p-1'>تعداد: {buyItemCount}عدد</button>}

              {buyItemCount !== 0 && <button disabled={item.mojodi === 0} onClick={() => {
                if (buyItemCount >= item.mojodi) return alert("موجودی این کالا تمام شد");
                buyItem(item);
              }} className='w-6 disabled:bg-slate-300 cursor-pointer my-2 rounded-md bg-[#30303a] text-white text-sm p-1'>+</button>}
